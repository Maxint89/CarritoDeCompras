  ...
  do {
      ...
      if (nombreCliente === null) {
          ...
          if (nombreCliente.length < 3 || !isNaN(nombreCliente)) {
              ...
              alert(`Bienvenido/a a Indumentaria del Pacífico ${nombreCliente}, ¿en qué podemos ayudarlo?`);
              nombreValido = true;
          }
      }
  } while (!nombreValido);
  ...
  do {
      ...
      if (opcion > 0) {
          ...
          if (continuarComprandoRespuesta === "eliminar") {
              const prendaAEliminar = prompt("Ingrese el nombre del tipo de prenda que desea quitar del carrito").toLowerCase();
              eliminarPrenda(prendaAEliminar, totalGastado);
          } else if (continuarComprandoRespuesta === "salir") {
              seguirComprando = false;
          } else {
              alert("Seleccione una opcion valida. 'eliminar' para quitar un tipo de prenda del carrito o 'salir' para concluir la compra");
          }
      } else {
          alert("Opcion invalida. Por favor seleccione una opcion entre 1 y 5 para comprar o 0 para salir o eliminar un producto del carrito");
      }
